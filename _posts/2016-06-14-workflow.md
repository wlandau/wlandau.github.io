...
<p>
If you have time-consuming workflow, it may be prudent to try a scaled-down
dry run first. If you intersperse your code with enough calls to <a href="https://github.com/wlandau/downsize"><code>downsize()</code></a>, 
it is trivially easy to scale the workload up or down. 
That way, your entire workflow becomes a 
quick unit test with as little change as possible. 
</p>

<p>Say you want to analyze a large dataset.
...
<pre><code>> small_data <- head(big_data)
</code></pre>
...
The <code>downsize()</code> function provides a choice between datasets.
It executes <code>my_data <- big_data</code> if <code>getOption("downsize")</code> is <code>FALSE</code> or <code>NULL</code> (default) and 
<code>my_data <- small_data</code> if <code>getOption("downsize")</code> is <code>TRUE</code>. 
You can toggle the global option <code>downsize</code> with a call to <code>scale_up()</code> or <code>scale_down()</code>, and you can check the status with `scaling()`.
...
In this case, <code>my_data <- downsize(big_data, small_data)</code> is equivalent to <code>my_data <- downsize(big_data, nrow = 6)</code>. You can override the global option <code>downsize</code> with <code>downsize(..., downsize = L)</code>, where <code>L</code> is <code>TRUE</code> or <code>FALSE</code>. More functionality like this is demonstrated in the <a href="https://github.com/wlandau/downsize/blob/master/README.md">README</a>.
</p>
