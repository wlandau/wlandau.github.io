...
The main selling point of <a href="https://www.gnu.org/software/make/">GNU Make</a>
is that the intermediate stages of a given project are (re)built if and only
...
<a href="http://yaml.org/">YAML</a> to

<ul>
<li>Generates some datasets.</li>
<li>Analyze each dataset with multiple methods.</li>
<li>Summarize each analysis with each dataset.</li>
<li>Output tables and plots from the summaries.</li>
...
<p>
To distrubite the workflow over multiple simultaneous
...
parallel processes. As before, changes to the functions in <code>code.R</code> 
trigger rebuilds for the targets that need updating, and the <code>recall()</code> and
...
full scaled-up workflow right away. I first run a scaled-down
...
For atomic objects and data frames, setting <code>random</code> to <code>TRUE</code>
