...
The <code>ds()</code> function executes <code>my_data <- big_data</code> if the <code>getOption("downsize")</code> is <code>FALSE</code> (default) and <code>my_data <- small_data</code> otherwise. You can toggle the global option <code>downsize</code> with a call to <code>scale_up()</code> or <code>downsize()</code> (same as <code>scale_down()</code>), and you can check the status with `scaling()`.
